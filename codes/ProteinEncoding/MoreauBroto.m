...
%This function calculates the Normalized Moreau-Broto descriptor (Dim 8*nlag)
...
function [moreaubroto]=MoreauBroto(str,nlag)
len=length(str);
result=zeros(1,8*nlag);AC=zeros(1,nlag); ATS=zeros(1,nlag);
...
prop1=zscore(prop1,1);
 ...
 for d=1:nlag
     for j=1:(len-d)
       AC(d)=AC(d)+t1(j)*t1(j+d);
     end
     ATS(d)=AC(d)/(len-d);
 end
 result(1:nlag)=ATS;
 AC=zeros(1,nlag); ATS=zeros(1,nlag);
...
%AccNo. BHAR880101
 ...
 for d=1:nlag
     for j=1:(len-d)
       AC(d)=AC(d)+t2(j)*t2(j+d);
     end
     ATS(d)=AC(d)/(len-d);
 end
  result((nlag+1):(2*nlag))=ATS;
 AC=zeros(1,nlag); ATS=zeros(1,nlag);
%AccNo. CHAM820101
 ...
 for d=1:nlag
     for j=1:(len-d)
       AC(d)=AC(d)+t3(j)*t3(j+d);
     end
     ATS(d)=AC(d)/(len-d);
 end
 result((2*nlag+1):(3*nlag))=ATS;
 AC=zeros(1,nlag); ATS=zeros(1,nlag);
 ...
 for d=1:nlag
     for j=1:(len-d)
       AC(d)=AC(d)+t4(j)*t4(j+d);
     end
     ATS(d)=AC(d)/(len-d);
 end
  result((3*nlag+1):(4*nlag))=ATS;
 AC=zeros(1,nlag); ATS=zeros(1,nlag);
 ...
 for d=1:nlag
     for j=1:(len-d)
       AC(d)=AC(d)+t5(j)*t5(j+d);
     end
     ATS(d)=AC(d)/(len-d);
 end
  result((4*nlag+1):(5*nlag))=ATS;
 AC=zeros(1,nlag); ATS=zeros(1,nlag);
...
for n=1:len
 ...
 for d=1:nlag
     for j=1:(len-d)
       AC(d)=AC(d)+t6(j)*t6(j+d);
     end
     ATS(d)=AC(d)/(len-d);
 end
result((5*nlag+1):(6*nlag))=ATS;
 AC=zeros(1,nlag); ATS=zeros(1,nlag);
...
for n=1:len
 ...
 for d=1:nlag
     for j=1:(len-d)
       AC(d)=AC(d)+t7(j)*t7(j+d);
     end
     ATS(d)=AC(d)/(len-d);
 end
 result((6*nlag+1):(7*nlag))=ATS;
 AC=zeros(1,nlag); ATS=zeros(1,nlag);
...
for n=1:len
 ...
 for d=1:nlag
     for j=1:(len-d)
       AC(d)=AC(d)+t8(j)*t8(j+d);
     end
     ATS(d)=AC(d)/(len-d);
 end
result((7*nlag+1):(8*nlag))=ATS;
moreaubroto=result;
end